...
export const YoutubeEmbedConfig: CustomEmbedConfig = {
    ...
    parseUrl: async (url: string) => {
        // 원본 정규식에는 shorts/ 가 없어 YouTube Shorts 링크가 임베드되지 않았다.
        // (aphennet 기존 게시물에도 shorts 링크가 남아 있다)
        // `\/*` 는 `.../embed//ID` 처럼 슬래시가 중복된 URL 대응 (운영 게시물에 실존).
        // 캡처에서 `/` 를 제외하지 않으면 id 가 12자가 되어 아래 길이 검사에서 탈락한다.
        const match = /^.*(youtu\.be\/|shorts\/|live\/|v\/|u\/\w\/|embed\/|watch\?v=|&v=)\/*([^#&?/]*).*/.exec(url);
