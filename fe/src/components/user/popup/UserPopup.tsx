...
export default function UserPopup() {
    ...
    const { data: configData } = useGetUserPopupList(type, {
    ...
    useEffect(() => {
        if (configData) {
            ...
            const layerPopups = validPopups
                .filter((item: PopupListItem) => item.p_layer_pop.includes("1"))
                .map((item: PopupListItem) => ({
                    ...item,
                    p_top_point: item.p_top_point < 80 ? 80 : item.p_top_point,
                }));
